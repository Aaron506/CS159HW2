...
for t in range(0,maxTime): # Time loop
 	xt = sys.x[-1]
 	ut = ftocpSQP.solve(xt) # Aaron added
...
for t in range(0, 6):
 	...
 	plt.plot(xPred[t][:,0], xPred[t][:,1], '--.b', label='Predicted trajectory using one iteration of SQP at time $t = $'+str(t))
